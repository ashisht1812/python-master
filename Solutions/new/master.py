def prepare_table_data(self, report_column_info, dataframe, group_colors, multi_headers):
    ...
    # 1. Find the column index where actual (non-empty) headers begin
    first_header_idx = next((i for i, h in enumerate(multi_headers[0]) if h.strip() != ""), 0)
    ...
    for level in range(num_levels):
        ...
        while col_idx < num_columns:
            ...
            # Skip leading empty header cells before first_header_idx
            if col_idx < first_header_idx:
                row.append('')  # no Paragraph, just blank to preserve alignment
                col_idx += 1
                continue

            # Count how many times this header repeats (span right)
            span_count = 1
            while (
